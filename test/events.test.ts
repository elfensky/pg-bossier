...
test('subscribeEvents() returns events that emit "connected" on LISTEN', async () => {
  const events = await subscribeEvents(h.pool, SCHEMAS);
  // Most implementations emit 'connected' before subscribeEvents() resolves.
...
test('subscribeEvents() with already-aborted signal throws AbortError', async () => {
  const ac = new AbortController();
  ac.abort();
  await expect(subscribeEvents(h.pool, SCHEMAS, { signal: ac.signal })).rejects.toThrow(/abort/i);
});

test('two subscribers on the same pool both receive every event', async () => {
  const a = await subscribeEvents(h.pool, SCHEMAS);
  const b = await subscribeEvents(h.pool, SCHEMAS);
...
test('reconnect handles idle_session_timeout disconnect', async () => {
  ...
  const events = await subscribeEvents(timeoutPool, SCHEMAS);
  const log: string[] = [];
  events.on('connected', () => log.push('connected'));
  events.on('error', (ev) => log.push(`error:${ev.reason}`));
