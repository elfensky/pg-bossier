...
> **Status — `v0.7.0` tagged; first npm publish prepared but not yet live.** The package is publish-ready (prebuilt `dist/` in the tarball), but `npm publish` is held pending a descent-app validation pass — until then, install from the git tag (see [Install](#install)). Pre-1.0, so the API may still change between minors (semver) — v0.7.0 carries two such breaking changes (see [CHANGELOG](CHANGELOG.md)). All nine charter goals are delivered and validated across the descent-app adoption trials. Per-feature status is in [Features](#features) below; the full scope lives in [issue #1](https://github.com/elfensky/pg-bossier/issues/1).
...
pg-bossier is **not on npm yet** (the v0.7.0 publish is prepared but held pending
a descent-app validation pass). Until it lands, install from the git tag:

```bash
npm install 'git+https://github.com/elfensky/pg-bossier.git#v0.7.0'
```
...
**One import for everything.** `pg-bossier` re-exports pg-boss's whole surface, so `import { PgBoss, bossier, fromPrisma, type JobWithMetadata } from 'pg-bossier'` all resolve from the one package. Operational reads/writes (live state, prune, export, events, …) hang off the `bossier()` client rather than standalone exports — you don't hand-thread connections; the client does. Only `install` / `migrate` / `uninstall` are exported as standalone functions, since they run without a client.
...
`getInputSnapshot(jobId, attempt)` reads a specific attempt; `getLatestInputSnapshot(jobId)` reads the most-recent non-null snapshot across attempts. Two single-purpose methods, each with one return shape:
...
const result = await client.getLatestInputSnapshot<SpaceTrackPayload>(jobId);
