...
Each decision below is its own GitHub issue. Sub-issues opened during the issue #1 refinement. **Goal 1's issue ([#2](https://github.com/elfensky/pg-bossier/issues/2)) is closed — delivered by the storage substrate;** the rest were re-scoped on 2026-05-21 to reflect what the substrate settled. Goal 5's operational read API has since merged (PR #17) and its issue [#6](https://github.com/elfensky/pg-bossier/issues/6) is closed; Goal 6's persistent progress API has merged (`a7a8074`) and its issue [#7](https://github.com/elfensky/pg-bossier/issues/7) is closed; Goal 7's lifecycle event API has merged and its issue [#8](https://github.com/elfensky/pg-bossier/issues/8) is closed; the remaining goal issues stay open.
