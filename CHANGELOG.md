...
- **`recordPatch` `input_snapshot` semantics fixed.** `recordPatch({input_snapshot: null})` now actually clears the column (SQL NULL); previously this was a silent no-op due to `COALESCE`. Omitted field is unchanged (no-op). Routes through `stringifyOrThrow` for consistency with `recordInputSnapshot`.
